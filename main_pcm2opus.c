...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        if(len > 0)
        {
            ...
            char name[100];
            sprintf(name, "/home/tuan/libdatachannel/examples/streamer/samples/opus/sample-%d.opus", count);

            FILE *fp_sample = fopen(name, "wb");
            if (fp_sample) {
                fwrite(&len, 1, sizeof(len), fp_sample);
                fwrite(encoded_buff, 1, len, fp_sample);
            }
            fclose(fp_sample);
            count++;
        }
